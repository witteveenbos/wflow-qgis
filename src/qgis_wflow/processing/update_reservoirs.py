...
class UpdateReservoirsAlgorithm(AlgorithmBase):
    ...
    def process_algorithm(
        ...
        model = WflowSbmModel(root=input_path.parent, mode="r")

        reservoirs_gpkg, layer_name = path_from_feature_layer(reservoir_layer.source())
        reservoirs_gpd = gpd.read_file(filename=reservoirs_gpkg, layer=layer_name)

        model.setup_reservoirs_simple_control(reservoirs_fn=reservoirs_gpd, min_area=0.0)

        model.set_root(root=base_path, mode="w")
