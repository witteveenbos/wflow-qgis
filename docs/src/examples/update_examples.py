...
for file in os.listdir(os.path.join(__location__, EXAMPLES_BASE)):
    if file.endswith(".ipynb") and not os.path.exists(f'./{file.replace(".ipynb", "nblink")}'):
        with open(os.path.join(__location__, file.replace(".ipynb", ".nblink")), 'w') as nblink_file:
